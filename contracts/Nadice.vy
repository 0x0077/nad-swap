...
airdrop_over: public(bool)
game_over: public(bool)
latest_game_number: public(HashMap[address, uint256])


@deploy
def __init__(_name: String[25], _symbol: String[5]):
...
@external
def airdrop():
    assert not self.airdrop_over, "Airdrop Over"
    assert not self.is_claimed[msg.sender], "Claimed"
    self._mint(msg.sender, 888888 * 10 ** 18)


@external
def join_game(_number: uint256, _amount: uint256, _salt: uint256) -> uint256:
    assert not self.game_over, "Game Over"

    self._transfer(msg.sender, self, _amount)

    random_data: bytes32 = keccak256(
        abi_encode(
            _number,
            chain.id,
            _salt,
    ...
    random_number: uint256 = convert(
        keccak256(abi_encode(random_data, salt)),
        uint256
    ) 
    
    result: uint256 = random_number // 10**75

    if result < _number:
        multiplier: uint256 = (98 * 10000) // (_number * 10000) * 9900 
        bonus: uint256 = _amount * multiplier // 10000
        self._mint(msg.sender, bonus)
        self.latest_game_number[msg.sender] = result
...
@external
def set_airdrop_over(_is_over: bool):
    self._check_owner()
    self.airdrop_over = _is_over


@external 
def set_game_over(_is_over: bool):
    self._check_owner()
    self.game_over = _is_over